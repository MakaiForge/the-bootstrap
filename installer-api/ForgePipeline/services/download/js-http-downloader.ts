...
export class JsHttpDownloader {
  ...
  private _speedLimit: number | null = null;

  setMaxDownloadSpeedBytesPerSecond(limit: number | null): void {
    if (typeof limit !== "number" || !Number.isFinite(limit) || limit <= 0) {
      this._speedLimit = null;
    } else {
      this._speedLimit = Math.floor(limit);
    }
    void this._speedLimit;
  }
  ...
  private async executeDownload(
    ...
    const flags = startByte > 0 ? fs.constants.O_WRONLY | fs.constants.O_APPEND : fs.constants.O_WRONLY | fs.constants.O_CREAT | fs.constants.O_TRUNC;
    const fd = fs.openSync(actualFilePath, flags);
